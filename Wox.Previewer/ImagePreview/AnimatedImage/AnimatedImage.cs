...
namespace QuickLook.Plugin.ImageViewer.AnimatedImage
{
    public class AnimatedImage : Image, IDisposable
    {
        ...
        public static List<KeyValuePair<string[], Type>> Providers { get; set; } = new List<KeyValuePair<string[], Type>>();
        ...
        private static AnimationProvider InitAnimationProvider(Uri path, MetaProvider meta)
        {
            ...
            var provider = Activator.CreateInstance(type, path, meta) as AnimationProvider;

            return provider ?? new ImageMagickProvider(path, meta);
        }
        ...
        {
            var task = instance._animation.GetThumbnail(new Size(800, 600));
            if (task == null) return;
